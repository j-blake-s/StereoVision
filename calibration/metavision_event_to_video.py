...
def parse_args(argv=None):
    ...
    parser.add_argument('--file', type=str, default='', help='path to raw event file')
    parser.add_argument('--checkpoint', type=str, default='./e2v.ckpt', help='checkpoint to evaluate')
    ...
    parser.add_argument('--delta_t', type=int, default=40000, help='accumulate by delta_t')
...
def run(params):
    print('params: ', params)
    params.out_file = params.file[:-4] + "_reconstructed.mp4"

    if params.mode == 'adaptive':
        mv_it = AdaptiveRateEventsIterator(params.file, thr_var_per_event=params.thr_var)
    else:
        mv_it = EventsIterator(params.file, start_ts=params.start_ts, mode=params.mode,
                               n_events=params.n_events, delta_t=params.delta_t)

    window_name = "e2v"
    if params.no_window:
        window_name = None
    show_write = ShowWrite(window_name, params.out_file)
